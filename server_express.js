...
const uri= "mongodb+srv://Mayur:Mayur2006@cluster0.vzkri.mongodb.net/"
...
app.post('/students', async (req, res) => {
    try {
        ...
        const result = await students.insertMany(newStudent);
...
app.patch('/students/:rollNumber', async (req, res) => {
    try {
        ...
        if(result.modifiedCount==0){
            res.status(404).send("Student not found plz check rollnumber is exisit or not");
        }else{
        res.status(200).send(`${result.modifiedCount} document(s) updated`);
        }
    } catch (err) {
...
app.delete('/students/delete/:rollNumber', async (req, res) => {
    try {
        const rollNumber = parseInt(req.params.rollNumber);
...
app.delete('/students/delete/e/:name',async (req,res)=>{
    try{
        const name=req.params.name
        const result=await students.deleteOne({name});
        res.status(200).send(`${result.deletedCount} document deletd`);
    } catch(err){
        res.status(500).send("Error deleting student: "+err.message);
    }
});